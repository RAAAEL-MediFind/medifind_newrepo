...
"""
    ...
    with sidebar_drawer:
        ...
        with ui.column().classes("p-4 gap-y-2 h-full"):
            ...
            for label, route, icon_name in primary_links:
                with ui.row().classes(
                    "w-full items-center gap-4 px-3 py-2 rounded-lg hover:bg-sky-100 cursor-pointer"
            ...
            with ui.row().classes(
                "items-center gap-4 px-3 py-2 rounded-lg hover:bg-sky-100 cursor-pointer"
            ...
            with ui.row().classes("w-full gap-4 md:gap-6 flex-wrap overview-cards"):
                ...
                for key, title in card_titles.items():
                    with ui.card().classes(
                        "bg-white shadow rounded-xl p-4 border-t-4 border-sky-500 flex flex-col items-center text-center min-w-[150px]"
                    ):
                        ui.label(title).classes("text-gray-600 text-sm font-medium")
                        stat_labels[key] = ui.label("0").classes(
                            "text-3xl font-bold text-sky-700 mt-1"
                        )

            # --- "New Messages" Card (separate row) ---
            with ui.row().classes(
                "w-full gap-4 md:gap-6 flex-wrap mt-6"
            ):  # mt-6 adds space
                with ui.card().classes(
                    "bg-white shadow rounded-xl p-4 border-t-4 border-sky-500 flex flex-col items-center text-center min-w-[150px]"
                ):
                    ui.label("New Messages").classes(
                        "text-gray-600 text-sm font-medium"
                    )
                    stat_labels["total_messages"] = ui.label("0").classes(
                        "text-3xl font-bold text-sky-700 mt-1"
                    )

            ui.label("AVAILABLE STOCK").classes(
                "text-xl font-semibold text-sky-700 mt-8 mb-4"
            )
    ...
    def toggle_and_style(drawer: ui.left_drawer):
        ...
        drawer.classes(
            remove="open" if not sidebar_open_state else "",
            add="open" if sidebar_open_state else "open",
        )
        ui.run_javascript(
            f"document.body.classList.toggle('sidebar-open', {str(sidebar_open_state).lower()});"
        )
        ui.run_javascript(
            f"""
    ...
    async def fetch_data():
        ...
        try:
            ...
            stats_response = await asyncio.to_thread(
                requests.get, stats_url, headers=headers, timeout=10
            )
        ...
        try:
            ...
            stock_response = await asyncio.to_thread(
                requests.get, stock_url, headers=headers, timeout=15
            )
                    ...
                    for item in stock_items:
                        ...
                        with ui.card().tight().classes(
                            ...
                            with ui.card_section():
                                ...
                                ).classes("mb-1 text-xs")
                                ui.label(medicine_name).classes(
                                    "text-base font-semibold text-sky-700 leading-tight"
                                )
                                ...
                                ).classes("text-xs text-gray-500 my-1 h-8")
                                ui.label(f"GH₵ {item.get('price', 0):.2f}").classes(
                                    "text-sm text-gray-700 font-medium"
                                )
                                ui.label(f"Stock: {item.get('quantity', 0)}").classes(
                                    "text-xs text-gray-600"
                                )
                                with ui.row().classes("w-full justify-end gap-1 mt-1"):
                                    ui.button(
                                        icon="edit",
                                        on_click=lambda item_data=item: print(
                                            f"Edit {item_data}"
                                        ),
                                    ).props("flat round dense color=grey-8 size=sm")
                                    ui.button(
                                        icon="delete",
                                        color="red",
                                        on_click=lambda mid=medicine_id, mname=medicine_name: print(
                                            f"Delete {mid}, {mname}"
                                        ),
